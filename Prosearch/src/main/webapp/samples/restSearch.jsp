<%@ page contentType="text/json; charset=UTF-8"%><%@ include file="./api/common/ProEncrypt.jsp" %><%@ include file="./api/common/ProSearch.jsp" %><% request.setCharacterEncoding("UTF-8");%><%
	...
	*/
	...
	ProSearch proSearch = new ProSearch(isDebug); //search 객체 생성
	...
	* 조회할 모든 인덱스 세팅 *
	********************/
	String [] indexs = {"app","bbs","doc"} ;//검색대상 모든 index 필수 지정
	...
	for (int i = 0; i < indexs.length; i++) {
		proSearch.addIndex(indexs[i]);
	    proSearch.setAlias(indexs[i], indexs[i]);  
	    
	    
	    if("app".equals(indexs[i])){
	    	
	    }
	    
	    if("bbs".equals(indexs[i])){
	    	
	    }
	    
	    if("doc".equals(indexs[i])){
	...
	*******************/
    ...
    boolean isSearch = proSearch.doSearch(search, indexs);
