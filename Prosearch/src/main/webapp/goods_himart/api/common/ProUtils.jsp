<% request.setCharacterEncoding("UTF-8");%><%@ page import="java.util.*,java.text.DecimalFormat.*,java.text.*,java.util.regex.Matcher,java.util.regex.Pattern"%><%!
...
public static class ProUtils {
    ...
    public static String list2String(List<String> list, String split) {
        ...
        for ( int idx = 0 ; idx < list.size(); idx++) {
            ...
            sb.append(str);
            if ( idx +1 < list.size() ) sb.append(split);
        }
        return sb.toString();
    }
    ...
    /**
     ...
     public static String nvl(String name, String defaultValue) {
     ...
     public static String checkboxTrueFalseString(String [] params, String trueValue, String falseValue) {
     ...
     public static final  String [] getParameters(javax.servlet.http.HttpServletRequest request, String _param, int _size) {
     String [] params  = new String[_size];
     for ( int idx=0; idx<_size; idx++) {
     try {
     params[idx] = nvl(request.getParameter(_param + idx),"");
     } catch ( Exception e) {
     params[idx] = "";
     }
     }
    ...
    /**
     * Request 에 배열로 들어오는 내용들을 해쉬테이블에 담는다.
     * index별로 조건을 걸기 위한 기능으로 기본 구분자는 ; 이다.
     * index구분이 없으면 전체 index에 대해 조건이 부여된다.
     * ex) index1:value;index2:value
     * index 에 TOTAL 일 경우에는 전체 index에 대한 기준으로 판단한다.
     **/
     ...
     * request XSS 처리
     **/
    public static String getRequestXSS(javax.servlet.http.HttpServletRequest req, String parameter, String default_value) {
    ...
    public static LinkedHashMap<String,Object> setCustomSortAsMap(Map<String,Object> map, String [] fieldList) {
    	...
    	// 문자 길이로 sorting (오름차순)
        //Collections.sort(entries, (o1, o2) -> o1.getKey().compareTo(o2.getKey()));  //일부 자바버전은 람다식이 허용되지않음
        ...
        for (Map.Entry<String, Object> entry : entries) {
        	if(!Arrays.asList(fieldList).contains(entry.getKey().toLowerCase()) && !Arrays.asList(fieldList).contains(entry.getKey().toUpperCase())){
	        	resultMap.put(entry.getKey(), entry.getValue());        		
        	}
        } 
    	return resultMap;
    ...
    public static String getFileExt(String filename){
	 ...
	 /**
     * 한글자 단어 하이라이팅 처리 제거
     * @param fragment, stag, etag
     * @return
     */
    public static String getHighLightRemove(String tt) {
      Pattern pattern = Pattern.compile(HIGHLIGHT_STAG);
      Matcher matcher = null;
      String retText = "";
      String[] sarr = tt.split(" ");
      for (int i=0; i<sarr.length; i++) {
        matcher = pattern.matcher(sarr[i]); 
        if (matcher.find()) {
           if (sarr[i].contains(HIGHLIGHT_STAG) && sarr[i].contains(HIGHLIGHT_ETAG)) {
             String tmpTxt = sarr[i].substring(sarr[i].indexOf(HIGHLIGHT_STAG)+HIGHLIGHT_STAG.length(), sarr[i].indexOf(HIGHLIGHT_ETAG));
             if (tmpTxt.length() > 1) {
               retText += " " + sarr[i];
             } else {
               retText += " " + sarr[i].replace(HIGHLIGHT_STAG,"").replace(HIGHLIGHT_ETAG,""); 
             }
           } else {
              retText += " " + sarr[i];
           }
