...
FOS.item.rangeSlider.init = function (config, initJs) {
    ...
    if (config.showTicks && config.clickOnTicks) {
        ...
        ticksContainer.addEventListener('click', function (e) {
    ...
    */
    function getRange() {
        ...
        if (step == 'monthly' || step == 'yearly') {
            ...
            if (percStep < 100) {
                ...
                mid.forEach(function(val, index) {
    ...
    function dispatchChange(oldValue, newValue) {
        ...
        apex.event.trigger($("#"+config.itemName), CHANGE_EVENT, { oldValue:oldValue, newValue:newValue });
    }
